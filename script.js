...
const team=[
['JC','JULIANE COELHO BENO','DIRETORA GERAL / FUNDADORA','Liderança, visão sistêmica, biodiversidade e proteção do conjunto.'],
['JA','JOSÉ ADELINO GARCIA DOS SANTOS NETO','VICE-DIRETOR / DIRETOR EXECUTIVO','Execução, velocidade de decisão e coordenação estratégica.'],
['PH','PEDRO HENRIQUE FREITAS CORRÊA','DIRETOR DE DESIGN','Direção visual, contenção e expressão criativa.'],
['VH','VICTOR HUGO BELO MOTA','DIRETOR DE DESENVOLVIMENTO / PROGRAMADOR CHEFE','Tecnologia, arquitetura de sistemas e superação de obstáculos técnicos.'],
['GG','GUILHERME GOMES MARTINS','DIRETOR DE NARRATIVA E ROTEIRO','História, pistas, detalhes escondidos e construção de universo.'],
['AR','AMANDA RAQUEL PEREIRA DOS SANTOS','DIRETORA DE EDUCAÇÃO AMBIENTAL','Educação, transformação e recuperação ambiental.'],
['RS','RYAN SILVEIRA RAMOS','GAME DESIGNER','Jogabilidade, missões, desafios, recompensas e progressão.'],
['LC','LARISSA PIMENTA COSTA','DESIGNER DE MUNDO E CENÁRIOS','Território, cultura maranhense e composição do mundo.'],
['HC','HELOA DA SILVA COSTA','ESPECIALISTA EM TECNOLOGIA E INTELIGÊNCIA ARTIFICIAL','Energia, dados, tecnologia e inteligência aplicada.'],
['MF','MARIA FERNANDA COSTA MACHADO','DIRETORA DE MARKETING E COMUNICAÇÃO','Comunicação, posicionamento e conexão com o público.']
];
...
function setTeam(i){
 ...
 $('#teamInitials').textContent=t[0];$('#teamName').textContent=t[1];$('#teamRole').textContent=t[2];$('#teamDesc').textContent=t[3];
}
$('#profileBtn').addEventListener('click',()=>{
 const t=team[selectedTeam];$('#modalInitials').textContent=t[0];$('#modalName').textContent=t[1];$('#modalRole').textContent=t[2];$('#modalDesc').textContent=t[3];
 $('#modal').classList.add('show');$('#modal').setAttribute('aria-hidden','false');
});

const guardians=[
['GRD-01','LUNA','GUARDIÃ DA NATUREZA','REGENERAÇÃO','NATUREZA','RECUPERAÇÃO','Uma presença conectada à biodiversidade, recuperação e equilíbrio natural.'],
['GRD-02','ÍRIS','GUARDIÃ DA TECNOLOGIA','TECNOLOGIA NEXA','TECNOLOGIA','ANÁLISE','Integra dados e inovação para transformar informação em ação.'],
['GRD-03','ANDREW','GUARDIÃO DAS ÁGUAS','PURIFICAÇÃO','ÁGUA','RESTAURAÇÃO','Converte contaminação em um desafio de recuperação dos recursos hídricos.'],
['GRD-04','THEO','GUARDIÃO DA ENERGIA','ENERGIA NEXA','ENERGIA','OTIMIZAÇÃO','Representa eficiência, inovação energética e uso consciente de recursos.'],
['GRD-05','GUI','GUARDIÃO DA INVISIBILIDADE','MANTO ILUSÓRIO','INFORMAÇÃO','INFILTRAÇÃO','Investigação, pistas, detalhes e a força de enxergar o que está escondido.'],
['GRD-06','HENRIQUE','GUARDIÃO DO GELO ETERNO','PRISÃO GELADA','GELO','CONTENÇÃO','Simboliza impedir que um problema ambiental se espalhe antes de solucioná-lo.'],
['GRD-07','NETO','GUARDIÃO DA VELOCIDADE','TURBILHÃO RELÂMPAGO','VELOCIDADE','COLETA','Iniciativa, execução rápida e capacidade de agir antes que o problema cresça.'],
['GRD-08','VICTOR','GUARDIÃO DA FORÇA','IMPACTO TECTÔNICO','FORÇA','SUPERAÇÃO','Transforma capacidade técnica e resistência em ação física e estratégica.'],
['GRD-09','HELOA','GUARDIÃ DO SOL','LUZ SOLAR','ENERGIA LIMPA','INOVAÇÃO','Tecnologia, inteligência e energia limpa em uma só presença.'],
['GRD-10','AMANDA','GUARDIÃ DAS CHAMAS','FOGO ECOLÓGICO','TRANSFORMAÇÃO','EDUCAÇÃO','Usa a ideia de fogo como força de transformação e recuperação ambiental.'],
['GRD-11','LARISSA','GUARDIÃ DA MÚSICA','SINFONIA HARMÔNICA','CULTURA','HARMONIA','Reggae, tambores maranhenses e identidade territorial transformados em poder.'],
['GRD-12','JULIANE','GUARDIÃ DOS ANIMAIS','ESPÍRITO SELVAGEM','BIODIVERSIDADE','PROTEÇÃO','Liderança e cuidado com a vida conectados à proteção do conjunto.']
];
const gt=$('#guardianThumbs');let guardIndex=0;
guardians.forEach((g,i)=>{const b=document.createElement('button');b.className='guardian-thumb'+(i===0?' active':'');b.innerHTML=`<b>${g[1].slice(0,2)}</b><span>${g[1]}</span>`;b.onclick=()=>setGuardian(i);gt.appendChild(b)});
function setGuardian(i){guardIndex=i;const g=guardians[i];$$('.guardian-thumb').forEach((b,j)=>b.classList.toggle('active',j===i));$('#guardianCode').textContent=g[0];$('#guardianName').textContent=g[1];$('#guardianRole').textContent=g[2];$('#guardianPower').textContent=g[3];$('#guardianAttr').textContent=g[4];$('#guardianFunc').textContent=g[5];$('#guardianDesc').textContent=g[6];$('#guardianSilhouette').textContent=g[1][0];}
...
const villains=[
['VALTOR','SENHOR DO LIXO','Resíduos e descarte inadequado.'],
['STORMY','DAMA DA TEMPESTADE','Poluição atmosférica e instabilidade.'],
['OGRON','MONSTRO DAS ILUSÕES','Desinformação, fumaça e confusão.'],
['KABAL','BRUXO DO CÍRCULO NEGRO','Agente de caos ambiental.'],
['VEX','BRUXO DO CÍRCULO NEGRO','Engenharia do colapso.'],
['RYAN','BRUXO DO CÍRCULO NEGRO','Criador dentro da própria história.'],
['NÉVOA','IRMÃ MORTE','Poluição atmosférica.'],
['MACHADO','IRMÃ MORTE','Esgoto e águas residuais.'],
['TRITANUS','O GRANDE VILÃO','Acúmulo final da degradação.']
];
const vg=$('#villainGrid');villains.forEach(v=>{const c=document.createElement('article');c.className='villain-card';c.innerHTML=`<span class="villain-type">THREAT // ACTIVE</span><h3>${v[0]}</h3><p>${v[1]}</p><p>${v[2]}</p>`;vg.appendChild(c)});
...
['MISSÃO ESPECIAL','O CÍRCULO NEGRO','ÁREA INDUSTRIAL','Interferência de Kabal, Vex e Ryan.','GUI • VICTOR • NETO','Descobrir os planos do círculo.','🗺️ NOVA ÁREA'],
